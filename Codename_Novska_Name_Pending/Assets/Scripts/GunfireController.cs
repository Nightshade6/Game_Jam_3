...
namespace BigRookGames.Weapons
{
    public class GunfireController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            bulletScript = GetComponent<ProjectileController>();
        }
        ...
        {   
            ...
            //float xAcc = Random.Range(x , x );
            //float yAcc = Random.Range(y , y );

            //var ray = Camera.main.ScreenPointToRay(new Vector3(xAcc, yAcc, 0));

            //Rigidbody cloneBullet;

            //cloneBullet = Instantiate(projectilePrefab, muzzlePosition.transform.position, muzzlePosition.transform.rotation);
            //cloneBullet.velocity = bulletScript.speed * ray.direction;
        }

        private void ReEnableDisabledProjectile()
        {
            gn.currentAmmo = gn.maxAmmo;
            reloadSource.Play();
